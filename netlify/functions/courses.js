...
// There are 4 models: Courses_, _Lecturers__, Reviews__, _Sections____
// There is one many-to-many relationship: __Courses___ <-> __Lecturers___, which translates to two one-to-many relationships:
// - One-to-many: _Courses__ -> ____Section
// - One-to-many: _Lecturers____ -> ___Section__
// And one more one-to-many: _Reviews____ -> __Sections_
// Therefore:
// - The first model, ___Courses__, contains the following fields (not including ID): _Course number____, _Course Name____ 
// - The second model, __Lecturers___, contains the following fields: __Name___
// - The third model, ___Sections__, contains the following fields: __CourseId___, ___LecturerId__
// - The fourth model, __Reviews___, contains the following fields, _SectionId____, _Body___, __Rating__
...
exports.handler = async function(event) {
  ...
  let courseQuery = await db.collection(`courses`).where(`number`,`==`,courseNumber).get()
  
  // get the first document from the query
  let courses = courseQuery.docs[0]
  ...
  let sectionQuery = await db.collection(`sections`).where(`courseId`,`==`,courseId).get()
